...
document.addEventListener("DOMContentLoaded", () => {
  ...
  safeInit(qs(".product-details__thumbs-swiper") && qs(".product-details__main-swiper"), () => {
    const thumbsSelector = ".product-details__thumbs-swiper";
    const mainSelector = ".product-details__main-swiper";

    const getThumbsDir = () =>
      window.matchMedia("(max-width: 768px)").matches ? "horizontal" : "vertical";
  ...
  safeInit(qsa(".qty").length, () => {
    qsa(".qty").forEach((wrap) => {
      ...
      minus.addEventListener("click", () => setVal(Number(input.value || min) - 1));
      plus.addEventListener("click", () => setVal(Number(input.value || min) + 1));
